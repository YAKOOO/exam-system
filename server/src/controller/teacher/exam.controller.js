const service = require('../../service/teacher/exam.service')
...
class ExamController {
  ...
  async create(ctx, next) {
    ...
    const res = await service.create(exam);
  ...
  async update(ctx, next) {
    const exam = ctx.request.body;
    const res = await service.update(exam);
    const { warningStatus } = res
  ...
  async del(ctx, next) {
    ...
    const res = await service.del(paperId);
    const { warningStatus } = res
  ...
  async getList(ctx, next) {
    const { idList, name } = ctx.request.body;
    const res = await service.getList(idList, name, ctx.user.id)
    ctx.body = {
      respCode: 200,
      respMsg: '查询成功',
      list: res
  ...
  // 提交一个题目
  async subQuestion(ctx, next) {
    const ques = ctx.request.body;
    const res = await service.subQues(ques);
    const { warningStatus } = res
    if (warningStatus == 0) {
      ctx.body = {
        respCode: 200,
  ...
  async connectStu(ctx, next) {
    try {
      const { stuList, paperId} = ctx.request.body
      const res = await service.connectStu(stuList, paperId);
      ctx.body = {
        respCode: 200,
        respMsg: '试卷分发成功~'
      }
